 ...
 *     });
 */
Ext.define('Ext.ux.time.DateTimePicker', {
    ...
    renderTpl: [
        ...
        '</table>',
        '<tpl if="showToday">',
        '<div style="background-color:#ffffff;border-top:1px solid #99bce8;margin: 3px;padding: 5px 0 5px 0"><center>{%this.renderHour(values, out)%}{%this.renderMinute(values, out)%}{%this.renderSecond(values, out)%}</center></div>',
        '<div style="background-color:#ffffff;padding: 30px 0 5px 0"><center>{%this.renderTodayBtn(values, out)%}&nbsp;&nbsp;&nbsp;&nbsp;{%this.renderOkQueDingBtn(values, out)%}</center></div>',
        '</tpl>',
        ...
        '</div>',
        {
            ...
            renderHour: function (values, out) {
                out.push('<font  style="float : left; ">&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp</font>');
                Ext.DomHelper.generateMarkup(values.$comp.hour.getRenderTree(), out);
            },
            renderMinute: function (values, out) {
                out.push('<font  style="float : left;font-weight:bold;margin: 5px;">:</font>');
