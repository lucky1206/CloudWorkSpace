...
public class AutoCoderController {
    ...
    public JSONObject makeProject(
            ...
            @ApiParam(required = true, value = "Java实体模型包名，如：sys.entity、sys.models等", name = "epName") @RequestParam(name = "epName") String epName,
            @ApiParam(required = true, value = "Dao接口包名，如：sys.interface.idao、sys.dao等", name = "dpName") @RequestParam(name = "dpName") String dpName,
            @ApiParam(required = true, value = "Controller包名，如：sys.controller", name = "cpName") @RequestParam(name = "cpName") String cpName,
            ...
            @ApiParam(required = true, value = "Mapping映射文件目录名,如：sys.mapper.xml", name = "mfdName") @RequestParam(name = "mfdName") String mfdName,
            @ApiParam(required = true, value = "JS文件目录名，如：sys.jsentity、sys.jsmodel", name = "jsfdName") @RequestParam(name = "jsfdName") String jsfdName,
            @ApiParam(required = true, value = "JS文件名，如：sys.JsEntity、sys.JsModel", name = "jsfName") @RequestParam(name = "jsfName") String jsfName,
        ...
        pci.setInterceptorName(interceptorName);//拦截器包名
        ...
        try {
            ...
            // 2.1.2、实体模型包路径
            String entityPath = topPackPath + getSegDir(pci.getEpName());
            // 2.1.3、Dao接口类路径
            String daoPath = topPackPath + getSegDir(pci.getDpName());
            // 2.1.3-1、Dao接口实现类文件路径(框架中暂时不需要针对Dao接口的实现类)
            //String daoImplPath = topPackPath + getSegDir(pci.getDpName()) + pci.getDpNameImpl() + "\\";
            // 2.1.4、Controller控制器路径
            String controllerPath = topPackPath + getSegDir(pci.getCpName());
            // 2.1.6、Java实体合并打包为JS对象文件路径（此处代码移到上面的web工程配置中）
            // String jsDirPath = topPackPath + getSegDir(pci.getJsfdName());
            // 2.1.7、 环境监听器文件路径
            String listenerPath = topPackPath + "listener\\";
            // 2.1.8、 Swagger配置类文件路径
            String swaggerConfigPath = topPackPath + "swagger\\";
            // 2.1.9、Service业务层路径
            String servicePath = topPackPath + getSegDir(pci.getServName());
            // 2.1.9-1、Service业务层实现类文件路径
            String serviceImplPath = topPackPath + getSegDir(pci.getServName()) + pci.getServNameImpl() + "\\";
            // 2.1.10、Interceptor层路径
            String interceptorPath = topPackPath + getSegDir(pci.getInterceptorName());
            // 2.1.11、Utils工具集路径
            ...
            String mapperPath = topPackPath + getSegDir(pci.getMfdName());
            // 4、WEB-INF路径
     ...
     * @return
     */
    private String getSegDir(String dirs) {
        StringBuffer tempDir = new StringBuffer();
        if (dirs.contains(".")) {
            String[] ds = dirs.split("\\.");
            for (String e : ds) {
                tempDir.append(e).append("\\");
            }
        } else {
            tempDir.append(dirs).append("\\");
        }
    ...
    public JSONObject coding4Customize(@ApiParam(required = true, value = "工程配置整合参数", name = "pps") @RequestParam(name = "pps") String pps, @ApiParam(required = true, value = "接口关联表", name = "table") @RequestParam(name = "table") String table, @ApiParam(required = true, value = "查询字段", name = "queryFields") @RequestParam(name = "queryFields") String queryFields, @ApiParam(required = true, value = "条件字段", name = "conditionFields") @RequestParam(name = "conditionFields") String conditionFields, @ApiParam(required = true, value = "自定义接口参数配置", name = "apiConfig") @RequestParam(name = "apiConfig") String apiConfig, @ApiParam(required = true, value = "当前查询是否分页", name = "isPagging") @RequestParam(name = "isPagging") boolean isPagging, @ApiParam(required = true, value = "当前操作是否生成对应的JS模块", name = "isModuling") @RequestParam(name = "isModuling") boolean isModuling, @ApiParam(required = true, value = "当前操作是否生成web表格记录管理代码,如增删改", name = "isManaging") @RequestParam(name = "isManaging") boolean isManaging) throws IOException {
        ...
        if (queryFieldsArray != null && queryFieldsArray.size() > 0) {
            ...
            //模版解析及代码生成
            // 实体存放目录
            String javaFolder = ppsc.getEntityPath();
            ...
            if (reqName != null) {
                String jsFilePath = ppsc.getJsDirPath() + reqName + ".js";
                // 如js对象文件已经存在则删除
                ...
                // JavaScript存放目录
                File jsDir = new File(ppsc.getJsDirPath());
                if (!jsDir.exists())
                    jsDir.mkdirs();
            ...
            String mapperFolder = ppsc.getMapperPath();
            ...
            String daoFolder = ppsc.getDaoPath();
            ...
            String serviceFolder = ppsc.getServPath();
            ...
            String serviceImplFolder = ppsc.getServImplPath();
            ...
            String controllerFolder = ppsc.getControllerPath();
                    ...
                    if (reqArr != null && reqArr.size() > 0) {
                        ...
                        for (int i = 0; i < reqColsLen; i++) {
                            ...
                            if (colObj != null) {
                                ...
                                String dbType = colObj.getString("dbType");
                                ...
                                if (isLike) {
                                    formatedSql = formatedSql.substring(0, index) + formatedSql.substring(index)
                                            .replaceFirst("\\?", "CONCAT(CONCAT('%', #{" + pName + ",jdbcType=" + dbType + "}),'%')");
                                } else if (isIn) {
                                    formatedSql = formatedSql.substring(0, index) + formatedSql.substring(index)
                                    ...
                                    formatedSql = formatedSql.substring(0, index)
                                            + formatedSql.substring(index).replaceFirst("\\?", "#{" + pName + ",jdbcType=" + dbType + "}");
                                }
                            }
                        }
